...
画面遷移を実現するにあたり、KAMISHIBAIの最大の特徴は、モノリシックな一枚岩な構造ではなく、役割を分割して実現していることにあります。  
...
* 型安全性の保障された画面遷移時パラメーター  
* 画面遷移にともなう、一貫性と浸透性を伴ったイベント通知  

つぎのようなケースのイベント通知の実現は困難ですがKAMISHIBAIであれば一貫性を保った通知を実現します。  
